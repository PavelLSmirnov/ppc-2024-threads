...
TEST(smirnov_p_sort_double_batcher, array) {
  ...
  // Create Task
  SmirnovPavelStl::RadixSortDoubleBatcherSequential testRDSBSequential(RSDBDataSeq);
  ...
  // Create Task
  SmirnovPavelStl::RadixSortDoubleBatcherStlParallel testRDSBstlParallel(RSDBDataStl);
...
TEST(smirnov_p_sort_double_batcher, test_random) {
  // Create data
  std::vector<double> array = SmirnovPavelStl::randomVector(1000, -1000000, 1000000);
  // Create data
  std::vector<double> out_seq(array.size());
