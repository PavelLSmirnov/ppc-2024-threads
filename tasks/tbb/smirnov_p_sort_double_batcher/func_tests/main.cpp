...
#include "tbb/smirnov_p_sort_double_batcher/include/ops_tbb.hpp"
...
TEST(smirnov_p_sort_double_batcher, array) {
  ...
  // Create Task
  SmirnovPavelTbb::RadixSortDoubleBatcherSequential testRDSBSequential(RSDBDataSeq);
  ...
  // Create Task
  SmirnovPavelTbb::RadixSortDoubleBatcherTbbParallel testRDSBtbbParallel(RSDBDataTbb);
...
TEST(smirnov_p_sort_double_batcher, test_random) {
  // Create data
  std::vector<double> array = SmirnovPavelTbb::randomVector(1000, -1000000, 1000000);
  // Create data
  std::vector<double> out_seq(array.size());
